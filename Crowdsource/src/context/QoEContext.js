...
export const QoEProvider = ({ children }) => {
  ...
  const addStreamingSample = useCallback((sample) => {
    setMetrics((current) => {
      ...
      if (typeof bufferingCount === 'number') {
        next.data.streaming.bufferingCounts = [
          ...(next.data.streaming.bufferingCounts || []),
          bufferingCount,
        ];
      }
  ...
  const addFtpSample = useCallback((direction, sample) => {
    setMetrics((current) => {
      if (!['dl', 'ul'].includes(direction)) return current;
      const currentDirection = current.data.ftp[direction];
      const next = {
        ...current,
        data: {
          ...current.data,
      ...
      console.log(`[Data] FTP ${direction} sample:`, { request, completed, throughputKbps });

      if (request) next.data.ftp[direction].requests += 1;
      if (completed) next.data.ftp[direction].completed += 1;
      if (typeof throughputKbps === 'number') {
        next.data.ftp[direction].throughputs = [
          ...next.data.ftp[direction].throughputs,
          throughputKbps,
        ];
      }
  ...
  const addLatencySample = useCallback((sample) => {
    setMetrics((current) => {
      const next = {
        ...current,
        data: {
          ...current.data,
          latency: { ...current.data.latency },
        },
      };
      const {
        request = true,
